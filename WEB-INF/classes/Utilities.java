	...
	public void printHtml(String file) {
		...
		if (file == "header.html") {
			...
			if (session.getAttribute("username")!=null){
				...
				result = result +"<li><a href='Cart'><span class='glyphicon' style='font-size: initial'>Cart("+CartCount()+")</span></a></li></ul></div>";
				pw.print(result);
		} else
...
/*  CartCount Function gets  the size of User Orders*/
	...
	public void storeProduct(String name,String type,String maker, String acc){
		...
		if(type.equals("sdls")){
			SmartDoorlocks sdl;
			sdl = SaxParserDataStore.sdls.get(name);
			OrderItem orderitem = new OrderItem(sdl.getName(), sdl.getPrice(), sdl.getImage(), sdl.getRetailer());
			orderItems.add(orderitem);
		}
		if(type.equals("laptops")){
			SmartLightings laptop=null;
			laptop = SaxParserDataStore.sls.get(name);
			OrderItem orderitem = new OrderItem(laptop.getName(), laptop.getPrice(), laptop.getImage(), laptop.getRetailer());
			orderItems.add(orderitem);
		}
		if(type.equals("wts")){
			SmartDoorbells wt=null;
			wt = SaxParserDataStore.wts.get(name);
			OrderItem orderitem = new OrderItem(wt.getName(), wt.getPrice(), wt.getImage(), wt.getRetailer());
			orderItems.add(orderitem);
		}
		if(type.equals("vas")){
			SmartSpeakers va=null;
			va = SaxParserDataStore.vas.get(name);
			OrderItem orderitem = new OrderItem(va.getName(), va.getPrice(), va.getImage(), va.getRetailer());
			orderItems.add(orderitem);
		}
		if(type.equals("smttherms")){
			SmartThermostats smttherm=null;
			smttherm = SaxParserDataStore.smttherms.get(name);
			OrderItem orderitem = new OrderItem(smttherm.getName(), smttherm.getPrice(), smttherm.getImage(), smttherm.getRetailer());
			orderItems.add(orderitem);
		}
	...
	public HashMap<String, SmartDoorlocks> getsdls(){
	...
	public ArrayList<String> getProducts(){
		...
		for(Map.Entry<String, SmartDoorlocks> entry : getsdls().entrySet()){			
	...
	public HashMap<String, SmartLightings> getLaptops(){
	...
	public ArrayList<String> getProductsLaptop(){
		ArrayList<String> ar = new ArrayList<String>();
		for(Map.Entry<String, SmartLightings> entry : getLaptops().entrySet()){			
			ar.add(entry.getValue().getName());
		}
		return ar;
	}

	public HashMap<String, SmartDoorbells> getWearableTechs(){
	...
	public ArrayList<String> getProductsWearableTech(){
		ArrayList<String> ar = new ArrayList<String>();
		for(Map.Entry<String, SmartDoorbells> entry : getWearableTechs().entrySet()){			
			ar.add(entry.getValue().getName());
		}
		return ar;
	}

	public HashMap<String, SmartSpeakers> getVoiceAsst(){
	...
	public ArrayList<String> getProductsVoiceAsst(){
		ArrayList<String> ar = new ArrayList<String>();
		for(Map.Entry<String, SmartSpeakers> entry : getVoiceAsst().entrySet()){			
			ar.add(entry.getValue().getName());
		}
		return ar;
	}
